...
class Model(nn.Module):
	...
	def forward(self, x):
		...
		pred = self.tanh(self.linear(pred)).view(pred.data.shape[0], -1, 1)
		return pred

model = Model(input_dim, hidden_size, 1)
...
sample_num = 100
plt.plot(pred[sample_num].data.numpy(), label='Prediction')
...
act = torch.squeeze(activations['linear'][0], 2).detach().numpy()
act = act[:5].transpose()
plt.plot(act)
plt.show()
